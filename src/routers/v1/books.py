...
from src.schemas import IncomingBook, ReturnedAllBooks, ReturnedBookWithSeller
...
@books_router.post("/", response_model=ReturnedBookWithSeller, status_code=status.HTTP_201_CREATED)  # Прописываем модель ответа
...
@books_router.get("/{book_id}", response_model=ReturnedBookWithSeller)
...
async def update_book(book_id: int, new_data: ReturnedBookWithSeller, session: DBSession):
